    ...
    for idx in range(0, len(gt_pred_set)-nframes+1):
        ...
        for i, (gt_json, pred_json, gt_pan, pred_pan, gt_image_json) in enumerate(gt_pred_set[idx:idx+nframes]):
            ...
            if len(pred_labels_set) != 0:
                raise KeyError(
                    f"Segment IDs {list(pred_labels_set)} are presented in JSON and not presented in PNG.\n"
                    f"All PNG labels: {sorted(labels.tolist())}\n"
                    f"All JSON labels: {sorted([el['id'] for el in pred_json['segments_info']])}"
                )
...
def vpq_compute_parallel(gt_pred_split, categories, nframes, output_dir):
    start_time = time.time()
    vpq_stat = PQStat()
    ...
    f.write("{:4s}| {:>5s} {:>5s} {:>5s} {:>6s} {:>7s} {:>7s} {:>7s}\n".format("IDX", "PQ", "SQ", "RQ", "IoU", "TP",
                                                                               "FP", "FN"))
    for idx, result in results['per_class'].items():
        f.write("{:4d} | {:5.1f} {:5.1f} {:5.1f} {:6.1f} {:7d} {:7d} {:7d}\n".format(idx, 100 * result['pq'],
                                                                                     100 * result['sq'],
                                                                                     100 * result['rq'],
                                                                                     result['iou'], result['tp'],
                                                                                     result['fp'], result['fn']))
    if save_name:
        f.close()
...
def parse_args():
    ...
    parser.add_argument('--pred', type=str,
        help='test outout directory')
    parser.add_argument('--gt', type=str,
        help='ground truth directory', default='./cityscapes_vps_val_gt')
...
def main():
    args = parse_args()
    submit_dir = args.pred

    truth_dir = osp.join(args.gt, "panoptic_video")

    output_dir = osp.join(osp.dirname(submit_dir), "metrics")
    ...
    pan_gt_json_file = osp.join(args.gt, "panpotic_gt_val_city_vps.json")
    ...
    for idx, file in enumerate(files):
        gt_pans.append(os.path.join(truth_dir, file))
    ...
    for idx, file in enumerate(files):
        pred_pans.append(os.path.join(submit_dir, 'pan_pred', file))
    ...
    assert len(gt_pans) == len(pred_pans), "number of prediction does not match with the ground truth."
    ...
    print(f"----Final metrics-----\n"
          f"VPQ_all:   {sum(vpq_all)/len(vpq_all)}\n"
          f"VPQ_thing: {sum(vpq_thing)/len(vpq_thing)}\n"
          f"VPQ_stuff: {sum(vpq_stuff)/len(vpq_stuff)}")
