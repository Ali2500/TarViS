...
from tarvis.modelling.backbone.temporal_neck.pos_embeddings import PositionEmbeddingSine, PositionEmbeddingSine3D
from tarvis.modelling.backbone.temporal_neck.temporal_attention import TemporalAttentionLayer, get_patch_mask_indices
...
class TemporalNeck(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            common_stride: int,
            temporal_attn_patches_per_dim: int,
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ret = {}
        ret["input_shape"] = {
            k: v for k, v in input_shape.items() if k in cfg.PIXEL_DECODER.IN_FEATURES
        }
        ret["conv_dim"] = cfg.PIXEL_DECODER.CONVS_DIM
        ret["mask_dim"] = cfg.PIXEL_DECODER.MASK_DIM
        ret["norm"] = cfg.PIXEL_DECODER.NORM
        ret["transformer_dropout"] = cfg.PIXEL_DECODER.DROPOUT
        ret["transformer_nheads"] = cfg.PIXEL_DECODER.NHEADS
        ret["transformer_dim_feedforward"] = 1024  # use 1024 for deformable transformer encoder
        ret["transformer_enc_layers"] = cfg.PIXEL_DECODER.TRANSFORMER_ENC_LAYERS  # a separate config
        ret["transformer_in_features"] = cfg.PIXEL_DECODER.DEFORMABLE_TRANSFORMER_ENCODER_IN_FEATURES
        ret["common_stride"] = cfg.PIXEL_DECODER.COMMON_STRIDE
        ret["temporal_attn_patches_per_dim"] = 8
        return ret
...
# @GlobalRegistry.register("PixelDecoder", "m2f_timesformer_v1_2x")
# class MSDeformPixelDecoderLargePatchSize(MSDeformAttnPixelDecoder):
#     def __init__(
#         self,
#         input_shape: Dict[str, ShapeSpec],
#         *,
...
#             temporal_attn_patches_per_dim=temporal_attn_patches_per_dim,
