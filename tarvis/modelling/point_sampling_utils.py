# This module contains selected functions copied from detectron2/projects/pointrend
import torch
...
def point_sample(input: torch.Tensor, point_coords: torch.Tensor, **kwargs):
    """
...
def point_sample_3d(input: torch.Tensor, point_coords: torch.Tensor, **kwargs):
    """
    A wrapper around :function:`torch.nn.functional.grid_sample` to support 3D point_coords tensors.
    Unlike :function:`torch.nn.functional.grid_sample` it assumes `point_coords` to lie inside
    [0, 1] x [0, 1] x [0, 1] cube.
    Args:
        input (Tensor): A tensor of shape (N, C, T, H, W) that contains features map on a H x W grid.
        point_coords (Tensor): A tensor of shape (N, P, 3) that contains
    ...
    Returns:
        output (Tensor): A tensor of shape (N, C, P) that contains
            ...
            interpolation from `input` the same way as :function:`torch.nn.functional.grid_sample`.
            NOTE: To perform trilinear interpolation, use interpolation='bilinear' (weird pytorch thing)
    """
    add_dim = False
    if point_coords.dim() == 3:
        add_dim = True
